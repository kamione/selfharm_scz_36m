...
longitudial_change_fig %>% 
    ggsave(filename = here("outputs", "figs", "data-imputation_desc-longitudial_change.pdf"),
           height = 3,
           width = 6)
...
tvmvar_data$data <- data_long %>% select(Pos:case)
tvmvar_data$type <- c("g", "g", "g", "g", "g", "c")
tvmvar_data$level <- c(1, 1, 1, 1, 1, 2)
tvmvar_data$timepoint <-  tmp %>% select(index, month)
...
pdf(here("outputs", "figs", "data-imputation_tvmvar_lag1.pdf"), width = 12, height = 8)
