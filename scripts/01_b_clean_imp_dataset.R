...
sh_case_df <- main_df %>% 
    ...
    select(
        Ageat1stpre, Sex, Filter, Yrs_edu, Age_onset, 
        DUP_days, DUP_DSH_His, DUP_SS_His, sub_abuse, baseline_sa, Smoker4, 
        Schiz, EP1_hosp, dur_adm1, M1_Pos:M36_Pos,
        M1_Neg:M36_Neg, M1_Aff:M36_Aff, M1_SOFAS:M36_SOFAS,
        M1_compliance:M36_compliance, M1_dsh:M36_dsh, M1_SS:M36_SS
    ) %>% 
    # drop subjects with NA in baseline variables
    drop_na(Ageat1stpre, Sex, Filter, Yrs_edu, Age_onset, DUP_days,
            DUP_DSH_His, DUP_SS_His, sub_abuse)

sh_case_df_imputated <- sh_case_df %>% 
    select(Ageat1stpre, Sex, Filter, Yrs_edu, Age_onset, 
           DUP_days, DUP_DSH_His, DUP_SS_His, sub_abuse,
           M1_Pos:M36_Pos, M1_Neg:M36_Neg, M1_Aff:M36_Aff,
           M1_SOFAS:M36_SOFAS, M1_compliance:M36_compliance) %>% 
    mice(., m = 5, seed = 1234) %>% 
    complete() %>% 
    as_tibble()

sh_case_df_combined <- sh_case_df %>% 
    select(
        -c(Ageat1stpre, Sex, Filter, Yrs_edu, Age_onset, 
           DUP_days, DUP_DSH_His, DUP_SS_His, sub_abuse,
           M1_Pos:M36_Pos, M1_Neg:M36_Neg, M1_Aff:M36_Aff,
           M1_SOFAS:M36_SOFAS, M1_compliance:M36_compliance)
    ) %>% 
    bind_cols(sh_case_df_imputated)


write_rds(sh_case_df_combined, file = here("cache", "data-imp_df.rds"))
# sh_case_df_combined <- read_rds(file = here("cache", "data-imp_df.rds"))

sh_case_df_combined <- sh_case_df_combined %>% 
    rowwise() %>% 
    ...
    mutate(
        ...
        mssd_pos = mssd(c_across(M1_Pos:M36_Pos), na.rm = TRUE),
        mssd_neg = mssd(c_across(M1_Neg:M36_Neg), na.rm = TRUE),
        mssd_aff = mssd(c_across(M1_Aff:M36_Aff), na.rm = TRUE),
        mssd_sofas = mssd(c_across(M1_SOFAS:M36_SOFAS), na.rm = TRUE),
...
# write the table
table_dsh_group_comparison <- sh_case_df_combined %>% 
...
table_dsh_group_comparison %>% 
    as_gt() %>% 
    gt::gtsave(filename = here("outputs", "tables", "data-imp_desc-dsh_group_comparison.html"))
...
table_dsh_group_comparison %>% 
    ...
    flextable::save_as_docx(
        path = here("outputs", "tables", "data-imp_desc-dsh_group_comparison.docx"),
        pr_section = sect_properties
    )

# comparing the self harm type
table_harm_category_comparison <- sh_case_df_combined %>% 
...
table_harm_category_comparison %>% 
    as_gt() %>% 
    gt::gtsave(filename = here("outputs", "tables", "data-imp_desc-self_category_comparison.html"))
...
table_harm_category_comparison %>% 
    ...
    flextable::fontsize(size = 10, part = "all") %>% 
    flextable::save_as_docx(
        path = here("outputs", "tables", "data-imp_desc-self_category_comparison.docx"),
        pr_section = sect_properties
    )


# generate monthly status of dsh/SS, 1 = YES, 0 = NO
preprocessed_df <- sh_case_df_combined
for (n in 1:36) {
...
write_csv(preprocessed_df, here("data", "processed", "data-imp_decs-selfharm_longitudinal.csv"))
write_rds(preprocessed_df, here("data", "processed", "data-imp_decs-selfharm_longitudinal.rds"))
