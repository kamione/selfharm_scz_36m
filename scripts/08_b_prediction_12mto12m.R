...
results_mat <- matrix(nrow = month_case_length * 2, ncol = 100) %>%
    ...
    mutate(month = rep(13:25, each = 2), .before = "iter_0001") %>% 
...
# save cache
write_rds(results_mat_rf, here("cache", "data-imp_model-rf_desc-prediction_12mto12m.rds"))
# results_mat_rf <- read_rds(here("cache", "data-imp_model-rf_desc-prediction_12mto12m.rds"))
results_mat_rf %>% 
...
plot_timeseries_boxplots(
    results_mat_rf,
    saveplot = TRUE,
    filename = "boxplot_model-rf_data-imp_desc-prediction_results_12mto12m.pdf"
)
