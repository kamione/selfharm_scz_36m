...
preprocessed_df <- here("data", "processed", "cf_selfharm_longitudinal.rds") %>% 
    ...
    rowwise() %>% 
    mutate(
        m12M7_case = ifelse(sum(c_across(M7_case:M18_case), na.rm = TRUE), 1, 0),
        m12M8_case = ifelse(sum(c_across(M8_case:M19_case), na.rm = TRUE), 1, 0),
        m12M9_case = ifelse(sum(c_across(M9_case:M20_case), na.rm = TRUE), 1, 0),
        m12M10_case = ifelse(sum(c_across(M10_case:M21_case), na.rm = TRUE), 1, 0),
        m12M11_case = ifelse(sum(c_across(M11_case:M22_case), na.rm = TRUE), 1, 0),
        m12M12_case = ifelse(sum(c_across(M12_case:M23_case), na.rm = TRUE), 1, 0),
        m12M13_case = ifelse(sum(c_across(M13_case:M24_case), na.rm = TRUE), 1, 0),
        m12M14_case = ifelse(sum(c_across(M14_case:M25_case), na.rm = TRUE), 1, 0),
        m12M15_case = ifelse(sum(c_across(M15_case:M26_case), na.rm = TRUE), 1, 0),
        m12M16_case = ifelse(sum(c_across(M16_case:M27_case), na.rm = TRUE), 1, 0),
        m12M17_case = ifelse(sum(c_across(M17_case:M28_case), na.rm = TRUE), 1, 0),
        m12M18_case = ifelse(sum(c_across(M18_case:M29_case), na.rm = TRUE), 1, 0),
        m12M19_case = ifelse(sum(c_across(M19_case:M30_case), na.rm = TRUE), 1, 0),
        m12M20_case = ifelse(sum(c_across(M20_case:M31_case), na.rm = TRUE), 1, 0),
        m12M21_case = ifelse(sum(c_across(M21_case:M32_case), na.rm = TRUE), 1, 0),
        m12M22_case = ifelse(sum(c_across(M22_case:M33_case), na.rm = TRUE), 1, 0),
        m12M23_case = ifelse(sum(c_across(M23_case:M34_case), na.rm = TRUE), 1, 0),
        m12M24_case = ifelse(sum(c_across(M24_case:M35_case), na.rm = TRUE), 1, 0),
        m12M25_case = ifelse(sum(c_across(M25_case:M36_case), na.rm = TRUE), 1, 0),
    ) %>% 
    ungroup()
...
results_mat <- matrix(nrow = month_case_length * 2, ncol = 100) %>%
    ...
    mutate(month = rep(7:25, each = 2), .before = "iter_0001") %>% 
...
write_rds(results_mat_rf, file = here("cache", "data-cf_model-rf_desc-prediction_6mto12m.rds"))
# results_mat_rf <- read_rds(here("cache", "data-cf_model-rf_desc-prediction_6mto12m.rds"))
...
results_mat_rf %>% 
    ...
    summarize(mean = mean(value, na.rm = TRUE),
              sd = sd(value, na.rm = TRUE)) %>% 
    gt::gt() %>% 
    gt::gtsave(filename = here("outputs", "tables", "mod-rf_data-cf_desc-roauc_6mto12m.html"))
...
results_mat_rf %>% 
    ...
    summarize(mean = mean(value, na.rm = TRUE),
              sd = sd(value, na.rm = TRUE)) %>% 
    gt::gt() %>% 
    gt::gtsave(filename = here("outputs", "tables", "mod-rf_data-cf_desc-accurary_6mto12m.html"))
...
plot_timeseries_boxplots(
    results_mat_rf,
    saveplot = TRUE,
    filename = "boxplot_model-rf_data-cf_desc-prediction_results_6mto12m.pdf"
)
