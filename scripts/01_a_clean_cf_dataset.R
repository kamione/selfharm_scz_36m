...
sh_case_df <- data %>% 
    ...
    mutate(
        ...
        mssd_pos = mssd(c_across(M1_Poscf:M36_Poscf)),
        mssd_neg = mssd(c_across(M1_Negcf:M36_Negcf)),
        mssd_aff = mssd(c_across(M1_Aff:M36_Aff)),
        mssd_sofas = mssd(c_across(M1_SOFAScf:M36_SOFAScf)),
        mssd_compliance = mssd(c_across(M1_compliance:M36_compliance))) %>% 
    ungroup() %>% 
    mutate(sh_case = if_else(n_total_dsh > 0, 1, 0)) %>% 
    mutate(sh_case = factor(sh_case, levels = c(0, 1), labels = c("Without Self-harm", "With Self-harm"))) %>% 
    left_join(supp_df %>% select(-c(Ageat1stpre, DUP_days, M1_Aff)), by = "HCS_code") %>% 
    mutate(Sex = factor(Sex, levels = c(1, 2), labels = c("Male", "Female")))


# generate monthly status of dsh/SS, 1 = YES, 0 = NO
sh_case_df_copy <- sh_case_df %>%
    mutate(Filter = factor(Filter, levels = c(0, 1), labels = c("SC", "EI"))) 
 
for (n in 1:36) {
    variable_name <- glue("M{n}_case")
    sh_case_df_copy <- sh_case_df_copy %>%
        mutate("M{n}_case_no" :=
            rowSums(
...
data_for_analysis %>% 
    ...
    bold_labels() %>% 
    as_gt() %>% 
    gt::gtsave(filename = here("outputs", "tables", "cf_dsh_ss_comparison.html"))

# write clean_data
write_csv(sh_case_df_copy, here("data", "processed", "cf_selfharm_longitudinal.csv"))
