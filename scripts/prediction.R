...
data <- here("data", "processed", "selfharm_longitudinal.csv") %>% 
    read_csv(col_types = cols()) %>% 
    select(HCS_code,
           Sex, Ageat1stpre, Yrs_edu, Age_onset, DUP_days,
...
results_mat <- matrix(nrow = 31 * 2, ncol = 10) %>%
    as_tibble() %>% 
...
write_rds(results_mat, here("predictionmodels_caches.rds"))
...
ggsave(fig,
       filename = here("outputs", "figs", "prediction_results.pdf"),
       width = 6,
       height = 4)
